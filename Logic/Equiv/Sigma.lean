...
def equivFst {α₁ α₂ : Type _} {β : α₁ → Type _} (e : Equiv α₁ α₂) : Equiv ((x₁ : α₁) × β x₁) ((x₂ : α₂) × β (e.rev x₂)) where
  ...
  fwd_eq_iff_rev_eq := by intro
    | ⟨x₁, y₁⟩, ⟨x₂, y₂⟩ =>
      constructor
      · intro | rfl => ext; exact e.rev_fwd ..; simp_cast; rfl
      · intro | rfl => ext; exact e.fwd_rev ..; simp_cast; rfl
...
def equivSnd {α : Type _} {β₁ : α → Type _} {β₂ : α → Type _} (e : (x : α) → Equiv (β₁ x) (β₂ x)) : Equiv ((x : α) × β₁ x) ((x : α) × β₂ x) where
  ...
  fwd_eq_iff_rev_eq := by intro
    | ⟨x₁, y₁⟩, ⟨x₂, y₂⟩ =>
      constructor
      · intro | rfl => ext; rfl; simp_cast; exact (e x₁).rev_fwd ..
      · intro | rfl => ext; rfl; simp_cast; exact (e x₂).fwd_rev ..
...
where
  ...
  h3 := {
    ...
    fwd_eq_iff_rev_eq := by intro
      | ⟨x₁, y₁⟩, ⟨x₂, y₂⟩ =>
        constructor
        · intro | rfl => ext; rfl; simp_cast; rfl
        · intro | rfl => ext; rfl; simp_cast; rfl
    }
